<template>
<div>
  <div id="conta" class="page-header clear-filter">
    <div 
    ...
    <div class="content">
      <div class="container container-fluid">
        <div class="col-md-5 ml-auto mr-auto">
          <card type="login fluid"  plain>
            <div slot="header" >
             <h1 class="h1-seo">Reserva tu cita</h1>
            </div>
            <form @submit.prevent="methodenviaraBackend">
            <fg-input v-model="citas.nombre" type="text"
              class="no-border input-lg"
              addon-left-icon="now-ui-icons users_circle-08"
              placeholder= "nombre"
            >
            </fg-input>
                <fg-input v-model="citas.nombreAcompañante"
              class="no-border input-lg"
              addon-left-icon="now-ui-icons users_circle-08"
              placeholder="Nombre del acompañante (si aplica)"
            >
            </fg-input>
            <fg-input v-model="citas.fecha">
               <Datepicker id="day" placeholder="Selecciona una fecha" lang="es"/>
            </fg-input>
        
            <fg-input v-model="citas.correo"
              class="no-border input-lg"
              addon-left-icon="now-ui-icons ui-1_email-85"
              placeholder="Email"
            >
            </fg-input>
            <fg-input v-model="citas.celular"
              class="no-border input-lg"
  ...
  data(){
    ...
    methods: {
        methodenviaraBackend(){
            axios.post("http://localhost:8000/reservar", this.citas).then(()=>console.log("agendamiento creado"));
        }
    }
};
</script>
...
#day{
  ...
  --v-calendar-picker-color: #1687A7;
  --v-calendar-input-bg-color:#ffffff;
  --v-calendar-select-bg-color: #1687A7;
  --v-calendar-border-color: #323B43;
  --v-calendar-triangle-color: #1687A7;
  ...
  --v-calendar-range-text-color: rgb(255, 255, 255);
